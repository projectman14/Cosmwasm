...
pub fn execute(
    ...
    match msg{
        ExecuteMsg::CreatePoll { question , option1 , option2 } => execute_create_poll(deps, env, info, question , option1 , option2),
        ExecuteMsg::Vote { question, choice } => execute_vote(deps, env, info, question, choice),
    }
}

fn execute_create_poll(deps:DepsMut , _env:Env , _info: MessageInfo , question:String , _option1:String , _option2:String) -> Result<Response , ContractError>{

    if POLLS.has(deps.storage , question.clone()){
        return Err(ContractError::CustomError { val: "Key already taken!".to_string() });
    }

    let poll = Poll{ question: question.clone(), option1_votes:0 , option2_votes:0 };
...
fn execute_vote(deps:DepsMut , _env:Env , _info: MessageInfo , question:String , choice:String) -> Result<Response , ContractError>{
    ...
    let mut poll: Poll = POLLS.load(deps.storage, question.clone())?;
...
mod tests {
    ...
    fn test_create_poll(){
        ...
        let msg = ExecuteMsg::CreatePoll { question: "Do you love the coding".to_string() , option1: "yes".to_string() , option2: "no".to_string() };
        ...
        assert_eq!(get_polls_response , GetPollResponse {poll: Some(Poll { question: "Do you love the coding".to_string(), option1_votes: 0, option2_votes: 0 })})
    }
    ...
    fn test_vote(){
        ...
        let msg = ExecuteMsg::Vote { question: "Do you love the coding".to_string(), choice: "1".to_string() };
        ...
        let resp = query(deps.as_ref(), env.clone(), msg).unwrap();
        let get_polls_response:GetPollResponse = from_json(&resp).unwrap();
        assert_eq!(get_polls_response , GetPollResponse {poll: Some(Poll { question: "Do you love the coding".to_string(), option1_votes: 1, option2_votes: 0 })});

        let msg = ExecuteMsg::Vote { question: "Do you love the codi".to_string(), choice: "1".to_string() };
